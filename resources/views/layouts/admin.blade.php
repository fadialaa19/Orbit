...
@if(auth()->user()->role === 'super_admin' || in_array('dashboard', auth()->user()->permissions ?? []))            <a href="{{ url('/admin/dashboard') }}"
               class="group flex items-center gap-4 px-4 py-3.5 rounded-[1.3rem] font-bold text-sm transition-all {{ request()->is('admin/dashboard') ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600 hover:bg-slate-50' }}">
...
@if(auth()->user()->role === 'super_admin' || in_array('students', auth()->user()->permissions ?? []))            <a href="{{ url('/admin/students') }}"
               class="group flex items-center gap-4 px-4 py-3.5 rounded-[1.3rem] font-bold text-sm transition-all {{ request()->is('admin/students*') ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600 hover:bg-slate-50' }}">
                ...
                <span>الطلاب</span>
                <span class="mr-auto px-2.5 py-1 bg-indigo-100 text-indigo-600 rounded-xl text-[10px] font-black">{{ \App\Models\User::where('role', 'student')->count() }}</span>
            </a>
            @endif
@if(auth()->user()->role === 'super_admin' || in_array('scholarships', auth()->user()->permissions ?? []))            <a href="{{ url('/admin/scholarships') }}"
               class="group flex items-center gap-4 px-4 py-3.5 rounded-[1.3rem] font-bold text-sm transition-all {{ request()->is('admin/scholarships*') ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600 hover:bg-slate-50' }}">
...
@if(auth()->user()->role === 'super_admin' || in_array('contacts', auth()->user()->permissions ?? []))            <a href="{{ url('/admin/testimonials') }}"
               class="group flex items-center gap-4 px-4 py-3.5 rounded-[1.3rem] font-bold text-sm transition-all {{ request()->is('admin/testimonials*') ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600 hover:bg-slate-50' }}">
...
@if(auth()->user()->role === 'super_admin' || in_array('admins', auth()->user()->permissions ?? []))            <a href="{{ url('/admin/settings') }}"
               class="group flex items-center gap-4 px-4 py-3.5 rounded-[1.3rem] font-bold text-sm transition-all {{ request()->is('admin/settings*') ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600 hover:bg-slate-50' }}">
...
@endif
        ...
        <div class="p-6 border-t border-slate-50 bg-slate-50/30">
            <div class="flex items-center gap-4 bg-white p-3 rounded-2xl shadow-sm border border-slate-100">
                <div class="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center text-white text-sm font-black uppercase">
            ...
            <div class="flex items-center gap-6 flex-1">
                ...
                <form action="{{ url('/admin/search') }}" method="GET" class="relative hidden md:block w-full max-w-md">
                    <input type="text" name="q" x-model="searchQuery" placeholder="بحث عن طالب، تذكرة أو عملية دفع..."
                           class="w-full bg-slate-100 border-2 border-transparent focus:bg-white focus:border-indigo-100 rounded-2xl px-11 py-2.5 text-xs font-bold transition-all">
            ...
            <div class="flex items-center gap-4">
                <div class="relative">
                    <button @click="notificationsOpen = !notificationsOpen" @click.away="notificationsOpen = false"
                            class="relative p-2.5 bg-slate-50 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-2xl transition">
                    ...
                    <div x-show="notificationsOpen" x-cloak
                        ...
                        <a href="{{ url('/admin/notifications') }}" class="block p-4 text-center text-[10px] font-black text-slate-400 hover:text-indigo-600 bg-slate-50/30">عرض كافة الإشعارات</a>
                    </div>
                </div>
