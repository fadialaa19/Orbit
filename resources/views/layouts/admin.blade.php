...
@if(auth()->user()->role === 'super_admin' || in_array('contacts', auth()->user()->permissions ?? []))            <a href="{{ url('/admin/testimonials') }}"
            ...
            @endif
@if(auth()->user()->role === 'super_admin' || in_array('communities', auth()->user()->permissions ?? []) || in_array('support', auth()->user()->permissions ?? []))            <a href="{{ url('/admin/communities') }}"
            ...
            @endif
@if(auth()->user()->role === 'super_admin' || in_array('official-documents', auth()->user()->permissions ?? []) || in_array('support', auth()->user()->permissions ?? []))            <a href="{{ url('/admin/official-documents') }}"
               class="group flex items-center gap-4 px-4 py-3.5 rounded-[1.3rem] font-bold text-sm transition-all {{ request()->is('admin/official-documents*') ? 'bg-gold-100 text-gold-600' : 'text-slate-600 hover:bg-slate-50' }}">
                ...
                <span>الأوراق الرسمية</span>
            </a>
            @endif
@if(auth()->user()->role === 'super_admin' || in_array('announcements', auth()->user()->permissions ?? []) || in_array('support', auth()->user()->permissions ?? []))            <a href="{{ url('/admin/announcements') }}"
               class="group flex items-center gap-4 px-4 py-3.5 rounded-[1.3rem] font-bold text-sm transition-all {{ request()->is('admin/announcements*') ? 'bg-gold-100 text-gold-600' : 'text-slate-600 hover:bg-slate-50' }}">
                <svg class="w-5 h-5 opacity-80" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z"/></svg>
                <span>لوحة الإعلانات</span>
            </a>
            @endif
@if(auth()->user()->role === 'super_admin' || in_array('xp', auth()->user()->permissions ?? []))            <a href="{{ url('/admin/xp') }}"
               class="group flex items-center gap-4 px-4 py-3.5 rounded-[1.3rem] font-bold text-sm transition-all {{ request()->is('admin/xp*') ? 'bg-gold-100 text-gold-600' : 'text-slate-600 hover:bg-slate-50' }}">
